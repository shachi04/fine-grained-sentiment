...
class PSL_kitchen{
	...
	public static void main(String[] args)
	{
		for (int d = 1;d<41;++d)
		{
				new File("/Users/girishsk/Documents/Shachi/CMPS209C/reviews/Results/kitchen_nounigram/fold"+d).mkdir()
		}

		
		for(int i = 0; i <40; ++i)
		{
			PSL_kitchen a = new PSL_kitchen()
			a.pslmodel(i);
		}
	}
	...
	void pslmodel(int cvSet)
	{
		...
		String writefolder = System.getProperty("user.home") + "/Documents/Shachi/CMPS209C/reviews/Results/kitchen_nounigram/"
		...
		m.add rule : (all(A) & ~negsentiment(A)) >> possentiment(A), weight:5
...
//		m.add rule : (noncontrast(A,B) & negsentiment(B) ) >> negsentiment(A)  , weight :10, squared : false
		...
		for (trainSet = 1 ; trainSet<=39;++trainSet)
		{
			...
			filename = 'data'+java.io.File.separator+'sentiment'+java.io.File.separator+'kitchen'+
...
//				testDataPartition.get(cvSet)), filename+"subjectivity_neg.csv");
		...
		InserterUtils.loadDelimitedDataTruth(data.getInserter(unigrampos,
				testDataPartition.get(cvSet)), filename+"unigram_kitchen_pos.csv");

		InserterUtils.loadDelimitedDataTruth(data.getInserter(unigramneg,
				testDataPartition.get(cvSet)), filename+"unigram_kitchen_neg.csv");
