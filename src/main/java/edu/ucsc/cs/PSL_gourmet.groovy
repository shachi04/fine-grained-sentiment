...
class PSL_gourmet{
	...
	public static void main(String[] args)
	{
		for (int d = 1;d<41;++d)
		{
				new File("/Users/girishsk/Documents/Shachi/CMPS209C/reviews/Results/gourmet_changedsq/fold"+d).mkdir()
		}

		
		for(int i = 0; i <40; ++i)
		{
			PSL_gourmet a = new PSL_gourmet()
			a.pslmodel(i);
		}
	}
	...
	void pslmodel(int cvSet)
	{
		...
		String writefolder = System.getProperty("user.home") + "/Documents/Shachi/CMPS209C/reviews/Results/gourmet_changedsq/"
...
//				testDataPartition.get(cvSet)), filename+"subjectivity_neg.csv");
		...
		InserterUtils.loadDelimitedDataTruth(data.getInserter(unigrampos,
				testDataPartition.get(cvSet)), filename+"unigram_gourmet_pos.csv");

		InserterUtils.loadDelimitedDataTruth(data.getInserter(unigramneg,
				testDataPartition.get(cvSet)), filename+"unigram_gourmet_neg.csv");
